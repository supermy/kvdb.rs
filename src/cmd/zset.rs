...
fn load_meta(ctx: &CommandContext, key: &[u8]) -> KvdbResult<Option<Metadata>> {
...
fn ensure_zset_meta(ctx: &CommandContext, key: &[u8]) -> KvdbResult<Metadata> {
    match load_meta(ctx, key)? {
        Some(meta) => {
...
fn read_zset_meta(ctx: &CommandContext, key: &[u8]) -> KvdbResult<Option<Metadata>> {
    match load_meta(ctx, key)? {
        Some(meta) => {
            if meta.data_type() != Some(DataType::ZSet) {
                return Err(KvdbError::Command(WRONGTYPE.to_string()));
...
fn member_subkey(user_key: &[u8], version: u64, member: &[u8]) -> Vec<u8> {
    subkey(user_key, version, member)
}

fn score_subkey(user_key: &[u8], version: u64, score: f64, member: &[u8]) -> Vec<u8> {
    ...
    subkey(user_key, version, &sub)
}

fn parse_score_subkey(key: &[u8]) -> Option<(f64, &[u8])> {
    let (_, _, sub) = crate::encoding::parse_subkey(key)?;
    if sub.len() < 8 {
        return None;
    }
...
fn zadd(ctx: &CommandContext, args: &[Bytes]) -> KvdbResult<RespValue> {
    ...
    let mut meta = ensure_zset_meta(ctx, &meta_key)?;
    ...
    for (member, score) in updates {
        let mkey = member_subkey(user_key, version, &member);
        ...
        if let Some(old_bytes) = existing {
            ...
            // 删除旧 score -> member 映射
            let old_skey = score_subkey(user_key, version, old_score, &member);
        ...
        let skey = score_subkey(user_key, version, score, &member);
...
fn zrange(ctx: &CommandContext, args: &[Bytes]) -> KvdbResult<RespValue> {
    ...
    let user_key = &args[0];
    let meta_key = metadata_key(user_key);
    let meta = match read_zset_meta(ctx, &meta_key)? {
        Some(m) => m,
...
fn zrangebyscore(ctx: &CommandContext, args: &[Bytes]) -> KvdbResult<RespValue> {
    ...
    let pairs: Vec<(f64, Vec<u8>)> = ctx
        ...
        .into_iter()
        .filter_map(|(k, _)| {
            let (score, member) = parse_score_subkey(&k)?;
...
fn zrem(ctx: &CommandContext, args: &[Bytes]) -> KvdbResult<RespValue> {
    ...
    let user_key = &args[0];
    let meta_key = metadata_key(user_key);
    let mut meta = match read_zset_meta(ctx, &meta_key)? {
        Some(m) => m,
    ...
    for member in &args[1..] {
        ...
        if let Some(old_bytes) = ctx.storage.get(CF_SUBKEY, &mkey)? {
            ...
            let skey = score_subkey(user_key, version, old_score, member);
...
fn zrank(ctx: &CommandContext, args: &[Bytes]) -> KvdbResult<RespValue> {
    ...
    let target = &args[1];
    let target_score = match ctx
        .storage
        .get(CF_SUBKEY, &member_subkey(user_key, meta.version, target))?
    {
...
fn zscore(ctx: &CommandContext, args: &[Bytes]) -> KvdbResult<RespValue> {
    ...
    match ctx
        .storage
        .get(CF_SUBKEY, &member_subkey(user_key, meta.version, &args[1]))?
    {
        Some(bytes) => {
...
fn collect_sorted_items(
    ...
    let prefix = subkey(user_key, version, &[]);
